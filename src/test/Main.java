...
public class Main implements jog.input.InputEventHandler, jog.network.ClientEventHandler, jog.network.ServerEventHandler {
	...
	private void draw() {
		...
		drawCircles(128 - 128, length, jog.graphics.BlendMode.ALPHA);
		
		jog.graphics.setColour(0, 0, 0);
		jog.graphics.circle(true, 344, 64 + Math.sin(Math.PI / 3) * length / 1.5, 64);
		drawCircles(344 - 128, length, jog.graphics.BlendMode.ADDITIVE);
		
		jog.graphics.setColour(255, 255, 255);
		jog.graphics.circle(true, 560, 64 + Math.sin(Math.PI / 3) * length / 1.5, 64);
		drawCircles(560 - 128, length, jog.graphics.BlendMode.SUBTRACTIVE);
		
		jog.graphics.setColour(255, 255, 255);
		jog.graphics.circle(true, 776, 64 + Math.sin(Math.PI / 3) * length / 1.5, 64);
		drawCircles(776 - 128, length, jog.graphics.BlendMode.MULTIPLICATIVE);
		...
		int scissorY = 256;
		jog.graphics.setScissor(32, scissorY, 128, 128);
		jog.graphics.setColour(255, 255, 255);
		jog.graphics.rectangle(true, 32, scissorY, 256, 256);
		jog.graphics.setScissor();
		
		length = 64;
		jog.graphics.push();
		jog.graphics.translate(640, 224);
		...
		if (multiplayerRole == "Server" && server.connected()) {
